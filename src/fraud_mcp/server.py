...
server: MCPServer = MCPServer(
    ...
    middleware=[structured_error_middleware],
)
...
def check_velocity_rules(
    account_id: Annotated[
        ...
        Field(
            description=(
                "Account identifier in the exact form 'ACC-####' (e.g. 'ACC-1013'). Copy it "
                "from a previous tool response or the user's message."
            ),
            pattern=r"^ACC-\d{4}$",
            examples=["ACC-1021"],
        ),
...
def flag_case(
    account_id: Annotated[
        str,
        Field(
            description="Account identifier in the exact form 'ACC-####'.",
            pattern=r"^ACC-\d{4}$",
        ),
    ],
    reason: Annotated[
